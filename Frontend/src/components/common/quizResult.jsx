 ...
 * Built from two separate pieces of data:
 *   - `quiz.questions`      -> text + options (student-safe shape, no correctIndex)
 *   - `submission.answers`  -> { questionId, selectedIndex, isCorrect, correctIndex?, explanation? }
 *
 * `correctIndex` and `explanation` are only present once the backend's
 * getMySubmission/submitQuiz/retakeQuiz responses include them (added via
 * gradeQuiz on the backend, safe to reveal only after grading). If they're
 * missing, those two UI blocks simply don't render — no crash either way.
 */
export default function QuizResult({ quiz, submission, onRetake, onBackToQuizzes }) {
  const passed = submission.passed;
  const answersByQuestionId = Object.fromEntries((submission.answers || []).map((a) => [a.questionId, a]));
    ...
    <div className="max-w-3xl mx-auto p-6">
      ...
      <div className="space-y-4 mb-6">
        {quiz.questions.map((q, qi) => {
          ...
          const isCorrect = answer?.isCorrect;
          const selectedText = answer ? q.options[answer.selectedIndex] : null;
          const correctText = answer?.correctIndex != null ? q.options[answer.correctIndex] : null;
          const explaination = answer?.explaination ?? null;   
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {!isCorrect && correctText && (
                    <p className="text-sm mt-1 flex items-center gap-1.5">
                      <CheckCircle2 size={14} className="text-emerald-500 flex-shrink-0" />
                      <span className="text-emerald-700">Correct answer: {correctText}</span>
                    </p>
                  )}

                  {explaination && (
                    <div className="mt-2 bg-[#faf7ff] border border-[#eee7ff] rounded-lg p-3">
                      <p className="text-xs font-semibold text-[#8b5cf6] mb-1">Explanation</p>
                      <p className="text-xs text-gray-600">{explaination}</p>
                    </div>
                  )}
                </div>
              </div>
