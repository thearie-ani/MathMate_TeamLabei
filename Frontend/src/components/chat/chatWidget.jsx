...
export default function ChatWidget() {
  ...
  const widgetWidth = expanded ? 520 : 420;
  const widgetHeight = expanded ? 720 : 640;

  return (
    <div style={{ position: "fixed", bottom: 20, right: 20, zIndex: 1000 }}>
      {open && (
        <div style={{
          width: widgetWidth, height: widgetHeight, marginBottom: 12,
          maxHeight: "85vh",
          background: "#1a1928", border: "1px solid #2d2b42",
          borderRadius: 12, display: "flex", flexDirection: "column",
          ...
          <div style={{ padding: 12, borderBottom: "1px solid #2d2b42" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
              <img
                ...
                style={{ width: 28, height: 28, borderRadius: "50%", objectFit: "cover" }}
              />
              <span style={{ color: "#fff", flex: 1 }}>MathMate Assistant</span>
              <button
                onClick={() => setExpanded(x => !x)}
                title={expanded ? "Shrink" : "Expand"}
                style={{ background: "none", border: "none", color: "#e6a8d7", cursor: "pointer", fontSize: 16 }}
              >
                {expanded ? "⤡" : "⤢"}
              </button>
            </div>
          ...
          {pendingImage && (
            <div style={{ padding: "8px 12px", borderTop: "1px solid #2d2b42", display: "flex", alignItems: "center", gap: 8 }}>
              <img src={pendingImage.previewUrl} alt="preview" style={{ width: 40, height: 40, objectFit: "cover", borderRadius: 6 }} />
              <span style={{ color: "#999", fontSize: 12, flex: 1 }}>Image attached</span>
              <button
                type="button"
                onClick={() => setPendingImage(null)}
                style={{ background: "none", border: "none", color: "#e6a8d7", cursor: "pointer" }}
              >
                ✕
              </button>
            </div>
          ...
          <form onSubmit={handleSubmit} onPaste={handlePaste} style={{ display: "flex", borderTop: "1px solid #2d2b42" }}>
            <label style={{ display: "flex", alignItems: "center", padding: "0 10px", cursor: "pointer", color: "#e6a8d7" }}>
              📎
            ...
            <input
              value={input}
              onChange={(e) => setInput(e.target.value)}
              placeholder="Ask a question or paste a quiz image..."
              style={{ flex: 1, padding: 10, background: "transparent", color: "#fff", border: "none", outline: "none" }}
            />
            <button type="submit" disabled={loading} style={{ padding: "0 14px", background: "none", border: "none", color: "#e6a8d7" }}>
              ➤
