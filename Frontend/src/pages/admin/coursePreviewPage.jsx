...
export default function CoursePreviewPage() {
  ...
  const { data: topics, isLoading: topicsLoading } = useQuery({
    ...
    queryFn: async () => (await courseApi.getTopicsByCourse(id)).data.data.topics,
    enabled: !!id,
  });
    ...
    <div className="p-6 max-w-4xl">
      <div className="flex items-center justify-between mb-4 p-3 bg-[#f3effe] border border-violet-200 rounded-xl flex-wrap gap-2">
        <div className="flex items-center gap-2 text-[#7c3aed] text-sm font-medium">
          <Eye size={15} />
          Admin Preview — this is what students see
        </div>
      ...
      <Card>
        <div className="p-5 border-b border-[#e8e4f8]">
          <h2 className="font-bold text-[#1a1535]">Topics ({topics?.length || 0})</h2>
        </div>
        <div className="divide-y divide-[#f0eeff]">
          {topicsLoading ? (
            ...
            <div className="p-8 text-center text-gray-400 text-sm">
              No topics yet.{' '}
              <button onClick={() => navigate(`/admin/topics/new?courseId=${id}`)} className="ml-1 text-[#8b5cf6] hover:underline">
                Add a topic
              </button>
            </div>
          ) : (
            topics?.map((topic, idx) => (
              <div key={topic._id} className="flex items-center justify-between p-4 hover:bg-[#faf9ff] transition-colors">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-[#1a1535]">{topic.title}</p>
                    <p className="text-xs text-gray-400">{topic.estimatedMinutes || 15} min read</p>
                  </div>
                </div>
                <button
                  onClick={() => navigate(`/admin/topics/${topic._id}/preview`)}
