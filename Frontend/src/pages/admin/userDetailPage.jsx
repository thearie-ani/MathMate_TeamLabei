...
export default function UserDetailPage () {
  ...
  const { data: submissions } = useQuery({
    ...
    queryFn: async () => {
      // Use quiz history endpoint filtered by student — depends on your backend
      // For now returns empty until implemented
      return [];
    },
  });
    ...
    <div className="p-6 max-w-3xl">
      ...
      <Card className="p-6 mb-4">
        <div className="flex items-start gap-5">
          <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-pink-200 to-violet-200
          flex items-center justify-center text-2xl font-bold text-violet-700 flex-shrink-0">
            {user?.name?.[0]?.toUpperCase()}
          </div>
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              <h2 className="text-lg font-bold text-[#1a1535]">{user?.name}</h2>
        ...
        <div className="grid grid-cols-3 gap-4 mt-5 pt-5 border-t border-[#e8e4f8]">
          {[
          ...
          <div className="mt-4 p-3 bg-orange-50 border border-orange-100 rounded-xl flex items-center gap-2">
            ...
            <div>
              <p className="text-sm font-semibold text-orange-800">
                {user.learningStreak.current} day streak
              </p>
              <p className="text-xs text-orange-600">
                Longest: {user.learningStreak.longest} days
              </p>
            </div>
          </div>
        )}
          ...
          <div className="divide-y divide-[#f0eeff]">
            {submissions.map((s) => (
              <div key={s._id} className="py-3 flex items-center justify-between">
                <span className="text-sm text-[#1a1535]">{s.quiz?.title}</span>
                <ScoreBadge score={s.percentage} />
              </div>
            ))}
          </div>
        )}
